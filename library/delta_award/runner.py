...
def _goal(inputs):
    ...
    lines = [
        ...
        "",
        "STEP 1 - WARM UP + LOGIN CHECK:",
        "Navigate to https://www.delta.com and wait 10 seconds.",
        "If you see 'Log In', sign in using credentials for www.delta.com and wait 5.",
        "If you already see your name or SkyMiles balance, stay signed in and continue.",
        "",
        "STEP 2 - NAVIGATE TO THE REAL BOOKING FORM:",
        f"Your VERY NEXT ACTION must be: navigate to {search_url}",
        "Do not try to build the search manually from the homepage.",
        "Do not open Advanced Search unless the page is missing the booking form entirely.",
        "",
        "STEP 3 - WAIT FOR FORM:",
        "Your VERY NEXT ACTION must be: wait 3",
        "",
        "STEP 4 - ENFORCE AWARD + FLEXIBLE DATES (CRITICAL):",
        "On the booking form, verify these three settings before searching:",
        "  - 'Shop with Miles' MUST be checked.",
        "  - 'My Dates are Flexible' MUST be checked.",
        f"  - 'Best Fares For' MUST be '{cabin_display}'.",
        "If any one is wrong, fix it before continuing.",
        "If 'Best Fares For' is a combobox/span instead of a native <select>, click it and click the visible option text.",
        "If you see cash/one-day results later, that means these settings were wrong and you must go back and fix them.",
        "Do NOT click 'Find Flights' until all three are correct.",
        "",
        "STEP 5 - SUBMIT SEARCH:",
        "Your VERY NEXT ACTION must be: click the red 'Find Flights' button.",
        "",
        "STEP 6 - WAIT FOR RESULTS (CRITICAL - DO NOT SKIP):",
        "Your VERY NEXT ACTION must be: wait 8",
        "This is a separate action. The page needs time to load the flexible-date results.",
        "",
        "STEP 7 - ENFORCE MILES VIEW ON RESULTS:",
        "If the results page shows 'Price in $USD' or cash prices, click the 'Miles' tab before taking the final screenshot.",
        "Do not accept a cash-price page as success.",
        "",
        "STEP 8 - TAKE SCREENSHOT:",
        "Your VERY NEXT ACTION must be: screenshot",
        "The correct page is the 7-day flexible-date award calendar with miles prices per day.",
        "",
        "STEP 9 - REPORT AND DONE:",
        "Your VERY NEXT ACTION must be: done",
        "From the final results screenshot, read the calendar prices and report:",
        "",
        ...
        "",
        "=== CRITICAL WARNINGS ===",
        "- STEP 2 (navigate) is MANDATORY. Always navigate to the URL even if the homepage looks filled.",
        "- STEP 4 MUST happen before STEP 5: Shop with Miles checked, My Dates are Flexible checked, Best Fares For set correctly.",
        "- If you skip Shop with Miles or My Dates are Flexible, Delta returns cash or one-day results. That is NOT success.",
        "- If the results page shows $USD selected, click Miles before the final screenshot.",
        "- DO NOT click through to a specific itinerary. Stay on the flexible-date results page.",
        "- DO NOT scroll on results pages unless needed to reveal the calendar controls.",
        "- Follow steps 1-9 EXACTLY in order. Each step is ONE action.",
    ]
    return "\n".join(lines)
