...
def _united_result_url_traveler_counts(url: str) -> list[int]:
    try:
    ...
    counts: list[int] = []
    act = (query.get("act") or [""])[0].strip()
    if act.isdigit():
        counts.append(int(act))

    px = (query.get("px") or [""])[0].strip()
    first_px_value = px.split(",", 1)[0].strip()
    if first_px_value.isdigit():
        counts.append(int(first_px_value))
    return counts


def _united_result_url_has_requested_travelers(url: str, travelers: int) -> bool:
    counts = _united_result_url_traveler_counts(url)
    if not counts:
        return True
    return all(count == travelers for count in counts)


def _united_result_url_has_requested_award_params(url: str, travelers: int) -> bool:
    try:
        parsed = urlparse(url)
    ...
    if "choose-flights" not in parsed.path:
        return True
    # `at=1` is United's award-travel flag and is sufficient to confirm award/
    # miles mode. United no longer emits the legacy `tqp=A` param on the result
    # URL, so requiring it caused an infinite "reopening requested award URL"
    # loop even when the page was genuinely in Miles mode (confirmed live: the
    # results page shows "Show price in: Miles" with at=1 and no tqp).
    if (query.get("at") or [""])[0].strip() != "1":
        return False
    if (query.get("act") or [""])[0].strip() != str(travelers):
        return False
    if not _united_result_url_has_requested_travelers(url, travelers):
        return False
    return True
...
def _close_stale_united_tabs(browser: Any, keep_page: Any | None, observations: List[str]) -> None:
    """Close leftover United/blank tabs so runs don't accumulate dozens of tabs.

    Each destination search runs in its own subprocess and opens a fresh page;
    without this, tabs (united.com results, united.business SSO redirects, and
    about:blank) pile up across destinations and runs. Single-owner CDP locking
    guarantees no other run is using this endpoint concurrently, so it is safe
    to reap every stale tab except the one we just acquired.
    """
    closed = 0
    try:
        contexts = list(browser.contexts)
    ...
    for context in contexts:
        try:
            ...
            continue
        for page in pages:
            if page is keep_page or not _page_is_live(page):
                continue
            try:
            ...
            if (
                ...
                try:
                    ...
                    pass
    if closed:
        observations.append(f"United tab cleanup: closed {closed} stale tab(s)")
    ...
    for attempt in range(1, attempts + 1):
        ...
        if context_pw is None:
            ...
            try:
                ...
                _close_stale_united_tabs(browser, page, observations)
...
def _is_united_otp_visible(page: Any) -> bool:
    return bool(
        _first_visible(
            [
                page.get_by_text(re.compile(r"verification code sent", re.IGNORECASE)),
                page.get_by_text(re.compile(r"enter code", re.IGNORECASE)),
                page.get_by_label(re.compile(r"enter code", re.IGNORECASE)),
                page.get_by_label(re.compile(r"verification code", re.IGNORECASE)),
            ]
        )
    )
...
def _is_sign_in_visible(page: Any) -> bool:
    if _is_united_otp_visible(page):
        return True
    return bool(
        _first_visible(
            [
...
def _ensure_united_result_travelers(
    page: Any,
    booking_url: str,
    travelers: int,
    observations: List[str],
    *,
    require_award_params: bool = False,
) -> bool:
    try:
        current_url = page.url or ""
    except Exception:
        return True
    if "choose-flights" not in current_url:
        return True
    has_requested_travelers = _united_result_url_has_requested_travelers(current_url, travelers)
    has_award_params = _united_result_url_has_requested_award_params(current_url, travelers)
    if has_requested_travelers and (not require_award_params or has_award_params):
        return True

    counts = _united_result_url_traveler_counts(current_url)
    observed = ", ".join(str(count) for count in counts) if counts else "unknown"
    observations.append(
        f"United result URL used {observed} traveler(s), expected {travelers}, "
        f"award_params={has_award_params}; "
        "reopening requested award URL"
    )
    try:
        page.goto(booking_url, wait_until="domcontentloaded", timeout=30000)
    except Exception as exc:
        observations.append(f"United traveler-count URL correction failed: {exc}")
        return False
    _wait_for_united_award_transition(page, observations, timeout_s=20)
    try:
        corrected_url = page.url or ""
    except Exception:
        return True
    corrected_travelers = _united_result_url_has_requested_travelers(corrected_url, travelers)
    corrected_award = _united_result_url_has_requested_award_params(corrected_url, travelers)
    if not corrected_travelers or (require_award_params and not corrected_award):
        observations.append(
            "United traveler-count URL correction did not produce the requested traveler count "
            "and award parameters"
        )
        return False
    return True
...
def _complete_homepage_award_flow(
    ...
    *,
    ...
    # NOTE: login is handled AFTER the search submit (the sign-in/miles gate at
    # the post-submit check below), then the search is re-issued via
    # _ensure_united_result_travelers. This matches the proven live sequence
    # (search -> handle login prompt -> re-search -> 7-day calendar). Do NOT add
    # a pre-search login here — it double-prompts and loops.
    ...
    if not _ensure_united_result_travelers(
        page,
        booking_url,
        travelers,
        observations,
        require_award_params=True,
    ):
        return {
            "mode": "live",
            "real_data": False,
            "matches": [],
            "booking_url": booking_url,
            "summary": "United result page used the wrong traveler count or award-search parameters.",
            "raw_observations": observations,
            "errors": ["United result URL did not match requested award search"],
        }
...
def _mark_unconfirmed_mixed_calendar(matches: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    annotated: List[Dict[str, Any]] = []
    for row in matches:
        next_row = dict(row)
        next_row["mixed_cabin"] = True
        next_row["mixed_cabin_unknown"] = True
        next_row["source"] = "united_7day_calendar_mixed_unconfirmed"
        notes = str(next_row.get("notes") or "").strip()
        caveat = "mixed-cabin filter not confirmed"
        next_row["notes"] = f"{notes}; {caveat}" if notes else caveat
        annotated.append(next_row)
    return annotated


def _normalize_united_taxes(value: Any) -> str:
...
def _calendar_matches_business_column(
    ...
    selected_calendar = [
        row for row in calendar_matches if str(row.get("date") or "") == selected_date_text
    ]
    selected_business = [
        row
        for row in detail_matches
        if str(row.get("date") or "") == selected_date_text
        and str(row.get("cabin") or "").lower().strip() == "business"
        and bool(row.get("cabin_confirmed"))
    ]
    for calendar_row in selected_calendar:
        calendar_miles = int(calendar_row.get("miles") or 0)
        calendar_taxes = _normalize_united_taxes(calendar_row.get("taxes"))
        for business_row in selected_business:
            if calendar_miles != int(business_row.get("miles") or 0):
                continue
            business_taxes = _normalize_united_taxes(business_row.get("taxes"))
            if calendar_taxes and business_taxes and calendar_taxes != business_taxes:
                continue
            return True
    return False


def _mark_calendar_cabin_confirmed(matches: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    annotated: List[Dict[str, Any]] = []
    for row in matches:
        next_row = dict(row)
        next_row["cabin_confirmed"] = True
        notes = str(next_row.get("notes") or "").strip()
        caveat = "business calendar confirmed against visible Business column"
        next_row["notes"] = f"{notes}; {caveat}" if notes else caveat
        annotated.append(next_row)
    return annotated
...
def _finalize_united_result_page(
    ...
    *,
    ...
    def collect_matches(
        ...
        allow_unconfirmed_mixed_calendar = _env_true(
            "OPENCLAW_UNITED_REPORT_UNCONFIRMED_MIXED_CALENDAR"
        )
        if cabin.lower().strip() in {"business", "first"} and view_config["sorted"]:
            ...
            if current_calendar_matches and not view_config["mixed_hidden"]:
                if allow_unconfirmed_mixed_calendar:
                    observations.append(
                        "United calendar fares found before mixed-cabin hide could be confirmed; "
                        "reporting them with a mixed-cabin caveat."
                    )
                    current_calendar_matches = _mark_unconfirmed_mixed_calendar(
        ...
        if current_calendar_matches:
            if _calendar_matches_business_column(
                current_calendar_matches,
                detail_matches,
                depart_date,
            ):
                observations.append(
                    "United calendar matched the visible Business column on the selected day; "
                    "marking 7-day calendar cabin as confirmed."
                )
                current_calendar_matches = _mark_calendar_cabin_confirmed(
                    current_calendar_matches
                )
            elif detail_matches:
                observations.append(
                    "United calendar did not match the visible Business column on the selected "
                    "day; leaving 7-day calendar cabin unconfirmed."
                )
            calendar_allow_mixed = allow_mixed or any(
                bool(match.get("mixed_cabin_unknown")) for match in current_calendar_matches
            )
            filtered_calendar_matches = _filter_united_matches(
                current_calendar_matches,
                cabin=cabin,
                allow_mixed=calendar_allow_mixed,
            )
    ...
    if not matches and state["skeletons"] > 0:
        ...
        _wait_for_award_results(page, timeout_s=45)
        state = _page_state(page)
        result_text = _collect_result_text(page)
        matches, detail_matches, calendar_matches = collect_matches(result_text)

    # Reset trick: if the results stream is still wedged on skeleton loaders
    # (United sometimes never serves data to an automated session), re-issue the
    # award search with the opposite cabin and then back to the requested cabin.
    # Flipping the `sc` cabin param resets United's server-side results state and
    # often un-sticks the 7-day calendar. Bounded to a single reset so it cannot
    # become the rapid-resubmit loop we just removed.
    if (
        not matches
        and cabin.lower().strip() in {"business", "first"}
        and state["skeletons"] > 0
        and state["flight_cards"] == 0
    ):
        opposite = "first" if cabin.lower().strip() == "business" else "business"
        observations.append(
            f"United {context_label} stalled on skeletons; cabin-toggle reset "
            f"({cabin} -> {opposite} -> {cabin})"
        )
        ...
        matches, detail_matches, calendar_matches = collect_matches(result_text)
        observations.append(
            f"United {context_label} after cabin-toggle reset: "
...
def _submit_united_otp(page: Any, observations: List[str]) -> tuple[bool, str]:
    ...
    otp_prompt_visible = _is_united_otp_visible(page)
    ...
    code_input = _first_visible(
        [
            ...
            page.locator("[role='dialog'] input[autocomplete='one-time-code']"),
            ...
            page.locator(
                ...
                ":not([type='password']):not([type='submit'])"
            ),
        ]
    )
    if not code_input and len(otp_digit_inputs) < 6:
        if otp_prompt_visible:
            return False, "United 2FA requested but code input was not visible"
        return True, ""
...
def _ensure_united_login(
    ...
    if not username_input and not password_input and switch_accounts:
        ...
        observations.append("United login switched to explicit account entry")
        ...
        if username_input and not _fill(username_input, username):
            return page, False, "Failed to enter United username after switching accounts"

    continue_btn = _first_visible(
        [
            page.get_by_role("button", name=re.compile(r"^continue$", re.IGNORECASE)),
            page.get_by_text(re.compile(r"^continue$", re.IGNORECASE)),
    ...
    if continue_btn and not password_input:
        _click(continue_btn)
        observations.append("United login submitted username step")
        refresh(2, timeout_s=10)
        if not _page_is_live(page):
...
def run(context: Dict[str, Any], inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    direct_enabled = _env_true("OPENCLAW_UNITED_ENABLE_DIRECT_PATH")
    browser_agent_error = ""
    ...
    if direct_enabled:
        ...
        if direct_result is not None and direct_result.get("real_data"):
            return direct_result
        if direct_result is not None:
            observations.append(
                "United direct award URL path did not return reliable data; trying homepage path."
            )
    else:
