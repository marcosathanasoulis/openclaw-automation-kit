...
def _earliest_searchable_depart_date(today: date | None = None) -> date:
    anchor = today or date.today()
    return anchor + timedelta(days=ANA_MIN_BOOKING_DAYS)
...
def _agent_only_goal(inputs: Dict[str, Any]) -> str:
    origin = inputs["from"]
    dest = inputs["to"][0]
    ...
    return "\n".join([
        ...
        "If there is a trip-type control, select One-way. If ANA forces a roundtrip shell, keep reading only the outbound itinerary for the requested date and do not relabel the result as roundtrip.",
        ...
        "  - If an award-type layer, modal, or authentication overlay blocks the itinerary, dismiss it or select the visible 'ANA Award' option so the itinerary section is readable.",
        "  - Read the Required mileage shown (e.g. 205,000 Miles)",
        "  - Read the flight details (flight number, departure time, arrival time)",
        "  - Do NOT continue to Passenger Information or payment.",
        "  - Take a screenshot",
        ...
        "If ANA shows a roundtrip shell, still report only the outbound itinerary for the requested date without relabeling it.",
        "",
...
def run(context: Dict[str, Any], inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    earliest_depart_date = _earliest_searchable_depart_date(today)
    ...
    if end < earliest_depart_date:
        error = (
            "ANA Mileage Club awards cannot be booked within 96 hours of departure; "
            f"requested window ends {end.isoformat()} but the earliest searchable date is {earliest_depart_date.isoformat()}."
        )
